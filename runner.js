...
export function runDomExample(code,host,{timeout=2000}={}){
  return new Promise(resolve=>{
    host.innerHTML='';
    ...
    iframe.srcdoc=`<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src 'none'; img-src data:; script-src 'unsafe-inline'; style-src 'unsafe-inline'"><style>body{font:14px/1.5 system-ui;padding:14px;margin:0;color:#171717;background:#fff}button,input{font:inherit}pre{white-space:pre-wrap}</style></head><body><div id="root"></div><script>
    ...
    const finish=result=>{if(settled)return;settled=true;clearTimeout(timer);cleanup();resolve(result)};
